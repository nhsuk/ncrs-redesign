...
const searchList = require("../../../data/search-folk.js");
...
module.exports = (router) => {
  router.post("/overviewtab/feedback/nhs-number-search-post", function (req, res) {
    ...
    if (!nhsNumber) {
      ...
      return res.redirect("/overviewtab/feedback/nhs-number-search");
    }

    res.redirect(`/overviewtab/feedback/search-results?nhs-number=${nhsNumber}`);
  });

  router.post("/overviewtab/feedback/basic-details-search", function (req, res) {
    ...
    if (Object.keys(req.session.data.errors).length) {
      return res.redirect("/overviewtab/feedback/basic-details-search");
    }
    ...
    res.redirect(`/overviewtab/feedback/search-results?${params}`);
  });

  router.post("/overviewtab/feedback/advanced-details-search", function (req, res) {
    const gender = req.body["gender"];
    ...
    if (Object.keys(req.session.data.errors).length) {
      return res.redirect("/overviewtab/feedback/advanced-details-search");
    }
  ...
  router.post("/overviewtab/feedback/postcode-search-post", function (req, res) {
    ...
    if (Object.keys(req.session.data.errors).length) {
      return res.redirect("/overviewtab/feedback/postcode-search-post");
    }
    ...
    res.redirect(`/overviewtab/feedback/postcode-search-results?${params}`);
  });


  router.post("/overviewtab/feedback/postcode-search-filter-post", function (req, res) {
    const search = req.body["searchfilter"];

    console.log('search: ' + search)

    res.redirect(`/overviewtab/feedback/patient_2`);
  });
  ...
  // for postcode search - returns set list of results
  router.get("/overviewtab/feedback/postcode-search-results", function (req, res, next) {
      const { gender, firstName, lastName, dob, dobTo, dobFrom, postcode } =
        req.query;
      const isPostcodeSearch = postcode
      const returnedPatientsList = {};

      for (const [patientNhsNumber, patient] of Object.entries(searchList)) {
  ...
  // for normal search
  router.get("/overviewtab/feedback/search-results", function (req, res, next) {
 ...
 // router.get("/overviewtab/feedback/patient_2", function (req, res, next) {
   // let formin = req.session.data['show'];
   
  
   
   // res.render("overviewtab/feedback/patient_2#radios-here") 
   //  });
  ...
  //for overview1
  // router.get("/overviewtab/feedback/overview", function (req, res, next) {
  ...
  //go to overview
  //  return res.render("overviewtab/feedback/overview", {
  //   backtoresults: searchLink,
  //  });
  // });

  //for overview2
  // router.get("/overviewtab/feedback/overview2", function (req, res, next) {
  //  let showsearch = req.query.search;
  //  if (showsearch === "true") {
  //   var searchLink = "true";
  //   } else {
  ...
  //  //go to overview2
  //    return res.render("overviewtab/feedback/overview2", {
  //    backtoresults: searchLink,
  //  });
  // });
  ...
  //go to overview2
  //  return res.render("overviewtab/feedback/patient_2", {
  //   backtoresults: searchLink,
  // });
  //  });

  // router.post("/overviewtab/feedback/patient_2", function (req, res) {
